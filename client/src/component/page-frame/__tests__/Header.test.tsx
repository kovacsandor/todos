...
jest.mock('@mui/material/AppBar', () => ({ children, position }: AppBarProps) => (
  <div>
    AppBar component
...
jest.mock(
  ...
  () =>
    ({ children, component, sx, variant }: OverrideProps<TypographyTypeMap, ElementType> & TypographyProps) =>
      (
        <div>
...
describe('Header', () => {
  ...
  test('Typography is configured correctly', () => {
    ...
    const toolbar = within(appBar).getByText(/Toolbar component/i);
    const typography = within(toolbar).getByText(/Typography component/i);
    within(typography).getByText(/Typography is configured with component div/i);
    within(typography).getByText(/typography is configured with sx \{"flexgrow":1\}/i);
