      ...
      return (
        <div>
          ...
          {mockReact.Children.map(
            children,
            (child, index) =>
              mockReact.isValidElement(child) &&
              mockReact.cloneElement(child, { onClick: onClick(index) }),
          )}
        </div>
      );
    },
...
jest.mock(
  ...
  () =>
    ({ icon, label, onClick }: BottomNavigationActionProps) =>
      (
...
jest.mock(
  '@mui/material/Paper',
  () =>
    ({ children, elevation, sx }: PaperTypeMap['props']) =>
      (
        <div>
          Paper
...
jest.mock('src/component/Page/useGetRoute', () => ({
...
describe('Footer', () => {
  ...
  describe('layout', () => {
    ...
    test('paper is correct', () => {
      ...
      screen.getByText(
        /sx is \{"position":"fixed","bottom":0,"left":0,"right":0,"zindex":2\}/i,
      );
    });
